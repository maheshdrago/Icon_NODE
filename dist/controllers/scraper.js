...
const scraper = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    const { url } = req.body;
    if (!url) {
        console.log("url");
    ...
    try {
        const result = yield scrapeAboutPage(url);
        return res.status(200).json(result);
    }
    catch (error) {
        console.error(`Unexpected error in scraper: ${error.message}`);
        next(error);
    }
});
